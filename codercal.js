...
const parse = (event, key) => {
    ...
    if (description.indexOf('%v') >= 0) {
        description = description.replace('%v', components.vars[random(12) % components.vars.length]);
    }

    if (description.indexOf('%t') >= 0) {
        description = description.replace('%t', components.tools[random(11) % components.tools.length]);
    }

    if (description.indexOf('%l') >= 0) {
        description = description.replace('%l', components.lengths[random(12) % components.lengths.length]);
    }
...
bot.on('inline_query', (query) => {
    if (query.query) {
        ...
        calText += '\n\n座位朝向：面向' + directions[random(2) % directions.length] + '写程序，BUG 最少。'
            ...
            + '\n女神亲近指数：' + stars[random(6) % stars.length];
